...
void  BSP_Ser_Init (CPU_INT32U  baud_rate)
{
    CPU_INT16U      sbr;
    CPU_INT08U      temp;
    UART_MemMapPtr  UART_base = UART1_BASE_PTR;
                                                                /* ------------------ INIT OS OBJECTS ----------------- */
    ...
    BSP_IntVectSet(BSP_INT_ID_UART1_RX_TX, BSP_Ser_ISR_Handler);
    BSP_IntEn(BSP_INT_ID_UART1_RX_TX);
...
void  BSP_Console_Enable (void)
{
    ...
    BSP_IntVectSet(BSP_INT_ID_UART1_RX_TX, BSP_Console_ISR_Handler);
    UART_C2_REG(UART_base) |= UART_C2_RIE_MASK;                 /* Enable the Receive not empty interrupt             */
    return;
}
